...
class Checker(CheckerBase):
    ...
    def run_checks(self):
        for source in self.dcp._list_cpl:
            ...
            [self.run_check(check, source, stack=asset_stack) for check in checks]
            ...
            [
                self.run_check(
                    ...
                    source,
                    asset,
                    stack=asset_stack + [asset[1].get("Path") or asset[1]["Id"]],
                )
    ...
    def metadata_cmp_pair(
        self, playlist, metadata, type_a, type_b, cmp=operator.eq, message=""
    ):
    ...
    def check_cpl_xml(self, playlist):
        ...
        check_xml(
            self, playlist["FilePath"], cpl["__xmlns__"], cpl["Schema"], self.dcp.schema
        )
    ...
    def check_cpl_empty_text_fields(self, am):
        ...
        if missing_fields:
            self.error(
                "Missing {} field(s)".format(", ".join(missing_fields)), "missing"
            )
    ...
    def check_cpl_reel_duration(self, playlist):
        ...
        for reel in playlist["Info"]["CompositionPlaylist"]["ReelList"]:
            ...
            if pic["Duration"] < edit or pic["IntrinsicDuration"] < edit:
                self.error("Reel {} last less than one second".format(reel["Position"]))
    ...
    def check_cpl_reel_duration_picture_subtitles(self, playlist):
        ...
        if self.dcp.schema == "Interop":
            cmp_op = operator.eq
            mess = "MainSubtitle and MainPicture Duration must match for " "Interop DCP"
        else:
            cmp_op = operator.ge
            mess = (
                "MainSubtitle Duration must less than or equal to that of "
    ...
    def check_cpl_reels_timed_text_coherence(self, playlist):
        ...
        if any(reels_subtitle) and not all(reels_subtitle):
            ...
            self.error(
                "Missing Subtitle track on reel(s) : {}".format(", ".join(bad_reels))
            )
    ...
    def check_assets_cpl_filename_uuid(self, playlist, asset):
        ...
        if "Path" in asset:
            ...
            cpl_uuid = asset["Id"]
            if file_uuid:
                compare_uuid(self, ("FILENAME", file_uuid), ("CPL", cpl_uuid))
